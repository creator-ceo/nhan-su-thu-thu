...
```bash
node bo-kham/kham.mjs dap <phiên> 3                  # ô bấm chọn — ghi số thứ tự
node bo-kham/kham.mjs dap <phiên> 3,6                # ô chọn nhiều
node bo-kham/kham.mjs dap <phiên> 12                 # ô đếm số
node bo-kham/kham.mjs dap <phiên> "kể nguyên văn…"   # ô kể bằng lời
node bo-kham/kham.mjs bo  <phiên>                    # bí, bỏ qua
node bo-kham/kham.mjs lui <phiên>                    # sửa câu vừa rồi
```
...
```bash
node bo-kham/kham.mjs quet <phiên>
```
...
```bash
node bo-kham/kham.mjs quet <phiên>    # đọc lại toàn bộ tín hiệu
node bo-kham/kham.mjs soan <phiên>    # sinh kham-ra/<…>/de-xuat.md
```

Rồi **đọc `bo-kham/luat-ke-don.md` TRỌN VẸN** và kê đơn theo đúng luật trong đó. Đọc kèm các trang bản khám tự liệt kê ở dòng *"Trang wiki bắt buộc đọc"*.
