 ...
 * `wiki/agent-harness-design.md` §2.
 *
...
const GOC = path.resolve(HERE, '..');                    // thư mục chứa `wiki/`
const coFile = (...p) => { const f = path.join(...p); return fs.existsSync(f) ? f : null; };
const NAP_DIR = coFile(HERE, 'nap') || path.join(GOC, 'kham-ra');   // Đoàn: owner-os/nap · member: kham-ra/
/* Lệnh in ra cho người ta gõ lại phải là đường dẫn THẬT của chỗ đang chạy. Đóng cứng
   ...
   chỉ lộ ra ở máy người khác. Tính từ thư mục họ đang đứng. */
const LENH = `node ${path.relative(process.cwd(), fileURLToPath(import.meta.url)) || 'kham.mjs'}`;
...
function docLuat() {
  ...
  /* Bản member: bỏ phần đầu trang (nhãn "file sinh ra") rồi lấy phần luật. Trang wiki bắt
     ...
     `chan-doan-cua-vao` (luật đã nằm trong chính file này), `business-metrics`, `fulfillment-system`. */
  return {
...
const KHAM_DIR = path.join(HERE, 'kham');
